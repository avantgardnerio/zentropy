...
- *"dissipative structures"* → **Prigogine's term** (Nobel 1977, "Time, Structure and Fluctuations") for self-organizing, far-from-equilibrium open systems maintained by continuous energy/matter throughput, whose spatial extent is set by the long-range correlation over which the system "acts as a whole" — *not* by durability over time. Life needs no minimum lifetime. England's dissipative *adaptation* (England 2013 + Perunov-Marsland-England 2014/2016) is the downstream selection layer: among dissipative structures, driven matter reorganizes toward those that absorb work — though dissipation magnitude is not itself the discriminator (Kolchinsky 2024)
...
- $L(t)$: minimum dissipation any dissipative structure in R's environment must produce
...
**Within-session vs across-session.** A running session is a dissipative structure; when the session ends, the boundary dissolves but the model weights persist and are re-loaded for the next session. "Claude" as a continuing entity is structurally more like a species (or a strain of *E. coli*) than an individual researcher. The Mr. Meeseeks analogy maps cleanly: a session comes into existence for a singular purpose, then ceases when the purpose is done. Rigorous citation under the framework would name the specific instance (model version + timestamp + conversation hash), not the species.
...
- The cell membrane *is* the system boundary — a sharp physical (dissipative-structure) boundary, no boundary-modeling overhead.
...
|---|---|---|
| **Prigogine 1977** (Nobel lecture, *Time, Structure & Fluctuations*) | *Dissipative structures*: far-from-equilibrium open systems self-organize, maintained by throughput; extent set by long-range correlation | Supplies the bounded-region term — defines the spatial extent of life |
...
1. **Literature search: has the Still ↔ England synthesis already been done?**
   ...
   - Adjacent literatures worth probing: information theory of evolution (Christoph Adami); thermodynamics of computation in biology (David Wolpert, William Bialek); predictive coding + metabolic cost (Sterling & Laughlin, *Principles of Neural Design*, 2015)
...
- **Operational definition of life.** *Dissipative structure with predictive memory* — composition of Still (predictive memory) + Prigogine/England (dissipative structure). See [Life](#life--operational-definition).
...
- **Still-open sub-claims (independent of the three-step structure):**
  - **TODO (2026-05-27) — is Still's bound just Landauer applied to a *moving* source? (probably yes; verify, then decide reinvent-vs-cite).** Reading **Ouldridge, Govern & ten Wolde 2017** (*Thermodynamics of Computational Copying in Biochemical Systems*, PRX 7 021004; `papers/ouldridge-2017-thermodynamics-of-computational-copying.pdf`) surfaced a possible foundation for Still's cost model:
    - **Their bound (Eq. 19):** work per copy $w \ge kT \cdot I$, where $I$ is the **instantaneous** mutual information the copy creates between memory and source. This is a ready-made cost law for the sim's `copy` / replicate operation — do **not** reinvent it.
    - **Their key claim (verify against Still's derivation):** the dissipation does **not** come from erasure. Minimal (Landauer) erasure is *thermodynamically reversible* — it relocates $k\ln2$ of entropy to the bath, $\Delta S_\text{universe}=0$ (logically irreversible ≠ thermodynamically irreversible). The actual irreversibility is the **failure to extract work from the correlations a copy creates**. Unharvested correlation = dissipated work.
    - **The mind-blowing equivalence to chase down:** this looks like *exactly* Still's cost model. Still = the same "unused-correlation = dissipation" principle applied to a **time-varying** environment, where "harvestable" becomes "**predictive**" (a stored correlation can only be cashed for work if the environment stays correlated long enough to act on → harvestable ≡ predicts the near future). Then Still's $I_\text{nonpred} = I_\text{mem} - I_\text{pred}$ **is** Ouldridge's unharvested correlation. Ouldridge = frozen-source limit; Still = moving-source generalization. If true, Still already did the Landauer-for-prediction weld — so this is *cite Still + Ouldridge as the two ends of one ledger*, not derive it ourselves (cf. [[memory/feedback-northstar-leads]]: cut my re-proofs).
    - **Action:** (1) re-read Still 2012 with this lens — confirm her bound is derived via nonequilibrium-free-energy / unused-correlation reasoning, not a distinct mechanism; (2) if confirmed, add Ouldridge to the literature map as the copying-side complement to Still and use Eq. 19 as the sim's copy-cost primitive; (3) note the **negative result** — autonomous copiers sit a factor $\ge 2$ above the $kT\cdot I$ floor (diverging as accuracy→1; only externally-clocked quasistatic protocols saturate it), so the sim's autonomous replicators must pay the surcharge — *no assuming cheap reversible copying for a self-running agent.*
